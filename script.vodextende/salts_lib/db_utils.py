...
class DB_Connection():
    ...
    def __init__(self):
        ...
        if kodi.get_setting('use_remote_db') == 'true':
            ...
            self.db_path = os.path.join(db_dir, 'DEATHScache.db')
        self.db_lib = db_lib
    ...
    def import_into_db(self, full_path):
        ...
        try:
            ...
            if self.progress:
                ...
                progress.create('VOD TVsupertuga', line2='Import from %s' % (full_path), line3='Importing 0 of %s' % (num_lines))
    ...
    def init_database(self, db_version):
        try:
            ...
            if db_version is not None and cur_version != db_version:
                ...
                self.progress.create('VOD TVsupertuga', line1='Migrating from %s to %s' % (db_version, cur_version), line2='Saving current data.')
                self.progress.update(0)
                self.__prep_for_reinit()
    
            logger.log('Building VOD TVsupertuga Database', log_utils.LOGDEBUG)
