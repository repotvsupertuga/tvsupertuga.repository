...
class movies:
	def __init__(self):
		...
		if self.tmdb_key == '' or self.tmdb_key == None:
			self.tmdb_key         = base64.b64decode('NmJjNDc0MjU3MGFhZTQ5MzQxOGM5OTg3MDNlZGY5Y2Y=')
		...
		self.tmdbchi_link         = 'http://api.themoviedb.org/3/list/13044?api_key=%s' % (self.tmdb_key)
		self.tmdbsports_link      = 'http://api.themoviedb.org/3/list/13043?api_key=%s' % (self.tmdb_key)
		self.tmdbspotlight_link   = 'http://api.themoviedb.org/3/list/13375?api_key=%s' % (self.tmdb_key)
		self.tmdbtragedy_link     = 'http://api.themoviedb.org/3/list/13553?api_key=%s' % (self.tmdb_key)
	...
	def movieDirectory(self, items):
		...
		for i in items:
			try:
				...
				if "tt" in imdb                     : sysmetalliq = "plugin://plugin.video.metalliq/movies/add_to_library_parsed/imdb/%s/direct.elysium.q" % imdb
				elif not tmdb == "0" or tmdb == None: sysmetalliq = "plugin://plugin.video.metalliq/movies/add_to_library_parsed/tmdb/%s/direct.elysium.q" % tmdb
				else                                : sysmetalliq = "0"
