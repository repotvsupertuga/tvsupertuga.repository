...
class Player(xbmc.Player):
    ...
    def onPlayBackStarted(self):
        ...
        if xbmcvfs.exists(self.pastaVideo):
            ...
            dialog = xbmcgui.Dialog().yesno('EUROPEIPTV', u'Já começaste a ver antes.', 'Continuas a partir de %s?' % (tempoAux), '', 'Não', 'Sim')
            if dialog:
    ...
    def adicionarVistoBiblioteca2(self):
        ...
        if not os.path.exists(ficheiro):
            ...
            f.write('')
            f.close()
            xbmc.executebuiltin("XBMC.Notification(EUROPEIPTV,"+"Marcado como visto"+","+"6000"+","+ self.logo+")")
