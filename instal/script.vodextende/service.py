    ...
    def onPlayBackStopped(self):
        ...
        if self.tracked:
            ...
            if pl.size() == 1 and pl[0].getfilename().lower().startswith(plugin_url):
                logger.log('Service: Clearing Single Item VOD TVsupertuga Playlist', log_utils.LOGDEBUG)
                pl.clear()
    ...
    if kodi.has_addon('plugin.program.super.favourites'):
        ...
        if active_plugin == kodi.get_id():
            if sf.getSetting('CONTEXT') == 'true':
                logger.log('Disabling Global CX while VOD TVsupertuga is active', log_utils.LOGDEBUG)
                was_on = True
                sf.setSetting('CONTEXT', 'false')
        elif was_on:
            logger.log('Re-enabling Global CX while VOD TVsupertuga is not active', log_utils.LOGDEBUG)
